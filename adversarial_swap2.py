...
anti_forgery_path = "C:/Users/lab/Desktop/Deepfake-Disruption/AntiForgery"
sys.path.append(anti_forgery_path)

SimSwap_model_path = "C:/Users/lab/Desktop/Deepfake-Disruption/SimSwap"
sys.path.append(SimSwap_model_path)
...
from utils import lab_attack_faceswap, lab_attack_faceswap3  # 적대적 공격 함수 불러오기
...
if __name__ == '__main__':
    ...
    # 2️⃣ 추가적인 옵션 설정 (main() 내부에서 수정 가능)
    additional_options = {
        "crop_size": 224,
        "name": "people",
        "Arc_path": "./SimSwap/arcface_model/arcface_checkpoint.tar",
        "pic_a_path": "./SimSwap/crop_224/6.jpg",
        "pic_b_path": "./SimSwap/crop_224/trump.jpg",
    ...
    img_a_adv, _ = lab_attack_faceswap3(img_id, img_att, model, epsilon=epsilon, iter=iter, output_dir = opt.output_path)
    # img_b_adv, _ = lab_attack_faceswap(img_id, img_att, model, epsilon=0.03, iter=1)

            # Perform face swapping (with attack)
    # img_fake_attack = model(img_a_adv ,img_att, latend_id, latend_id, True)
    
    img_id_adv_downsample = F.interpolate(img_a_adv, size=(112, 112))
    latend_id_adv = model.netArc(img_id_adv_downsample)
    latend_id_adv = latend_id_adv.detach().to('cpu')
    latend_id_adv = latend_id_adv / np.linalg.norm(latend_id_adv, axis=1, keepdims=True)
    latend_id_adv = latend_id_adv.to('cuda')
    
    img_fake_attack = model(img_a_adv, img_att, latend_id_adv, latend_id_adv, True)
    ...
    output_path=opt.output_path+f'result_combined_v2_4_e{epsilon}_{iter}.jpg'
            # Save images
